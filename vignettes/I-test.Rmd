...
title: "The $I$-test"
author: 
- name: "Orestis Loukas and Ho-Ryun Chung"
...
```{r}
if (! requireNamespace("Barnard", quietly = TRUE))
...
```
In contrast to the $t$-test the $I$-test detects a significant difference! Moreover, the $t$-test cannot account for the different study designs. The $I$-test reveals that the difference in study desings leads only to a small difference.
...
For this example we use the `iris` dataset. Here, we want to know whether the attribute `what = `r (what = "Sepal.Length")`` differs between the three species of irs. The standard approach of the "old statistics" is the one-factorial ANOVA. The validity of the results of an one-factorial ANOVA depends on the validity of two assumptions:
...
Formally speaking this invalidates the results of an one-factorial ANOVA, such that it should not be used. 
Nonetheless, the one-factorial ANOVA gives following result
```{r oneFactorialANOVA}
summary(aov(as.formula(paste(what, "~ Species")), data = iris))
```
It is unclear whether the low $p$-value originates from a violation of the null hypothesis and/or the unmet assumption of equal variances.
...
1. Convert the data into the entity representation using the function `toEntities`
   ```{r toEntities}
